...
public class AcAppService {
    ...
    public JSONObject applyCode() {
        System.out.println("apply code");
        JSONObject resp = new JSONObject();
        resp.put("appid", KobConstants.APPID);
        try {
            resp.put("redirect_uri", URLEncoder.encode(KobConstants.ACAPP_REDIRECT_URI, "UTF-8"));
    ...
    public JSONObject receiveCode(String code, String state) {
        ...
        if (Boolean.FALSE.equals(redisTemplate.hasKey(state))) {
            System.out.println("redis has not state" + state);
            return resp;
        }
        ...
        if (getString == null) {
            System.out.println("apply token false");
            return resp;
        }
        ...
        if (getString == null) {
            System.out.println("apply info false");
            return resp;
        }
